...
%% packages
\usepackage{color,verbatim,Sweave,url,xargs,amsmath,float,caption,graphicx,dcolumn}
\usepackage{booktabs}
...
Please read carefully each question and the related instructions. \textbf{Assume that there is only one correct answer per question, unless instructions indicate explicitly that there might be several}. \newline

If you have to round numbers in your computations, \textbf{we recommend rounding at the unit for volumes} (e.g. 674 units), \textbf{2 digits for amounts} (e.g. \texteuro 1,327.45), and \textbf{4 digits for percentages} (e.g. 0.4537 so 45.37\%). \newline

Roundings may sometimes lead to (relatively small) discrepancies when several answers are suggested, so \textbf{select the answer you believe is the best possible answer to the question}. If two answers are filled in when there is no indication that several answers might be correct, you will not receive credit for your answer. \newline

In addition, please note that \textbf{every question is self-sufficient and independent from any other question}. Some questions may have names in common, but this happens by pure chance and has no meaning whatsoever. They will all rely on different underlying data.  
